  ...
  const handleProcess = useCallback(async () => {
    ...
    try {
      ...
      if (output.success && output.result) {
        ...
        setStatus('complete');
        logToolUsage('text-color', file.name, {
          mode,
          threshold,
          color,
          fileSize: file.size
        ...
        <>
          ...
          <Card variant="outlined" size="lg">
            <div className="space-y-4">
              <div>
                <label htmlFor="text-color-picker" className="block text-sm font-medium mb-2">{t('colorLabel')}</label>
                <div className="flex items-center gap-4">
                  ...
                  <input
                    ...
                    disabled={isProcessing}
                    aria-label={t('colorHexLabel') || 'Color Hex Code'}
                  />
                </div>
              </div>
              ...
              <div>
                <label htmlFor="text-color-threshold" className="block text-sm font-medium mb-2">
                  {t('thresholdLabel') || 'Threshold'}: {threshold}
                </label>
                <input
                  id="text-color-threshold"
                  type="range"
                  min="0"
                  max="255"
                  value={threshold}
