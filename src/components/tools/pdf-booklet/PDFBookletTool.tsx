    ...
    const handleCreateBooklet = useCallback(async () => {
        ...
        try {
            ...
            if (output.success && output.result) {
                ...
                logToolUsage('pdf_booklet', file.name, {
                    fileSize: file.size,
                    gridMode,
                    paperSize,
                    orientation,
                    ...
                    `}
                        ...
                        <div>
                            ...
                            <input
                                ...
                                aria-label={tTools('pdfBooklet.paddingLabel') || 'Padding'}
                            />
                        </div>
                        ...
                        <div className="pt-4 border-t border-[hsl(var(--color-border))]">
                            ...
                            <div
                                className="relative bg-gray-100 rounded-[var(--radius-md)] p-4 flex items-center justify-center min-h-[200px]"
                            >
                                {/* Preview Paper */}
                                <div
                                    className={`bg-white shadow-lg border border-gray-300 relative ${orientation === 'landscape' ? 'w-[280px] h-[200px]' : 'w-[200px] h-[280px]'
                                        }`}
                                >
                                    {/* Grid Lines */}
                                    <style>{`
                                        .preview-grid-${uniqueId} {
                                            padding: ${padding / 4}px;
                                        }
                                    `}</style>
                                    <div
                                        className={`preview-grid-${uniqueId} absolute inset-0 grid gap-0.5 ${gridMode === '1x2' ? 'grid-cols-1 grid-rows-2' :
                                            gridMode === '2x2' ? 'grid-cols-2 grid-rows-2' :
                                                gridMode === '2x4' ? 'grid-cols-2 grid-rows-4' :
                                    ...
                                    >
                                        {Array.from({ length: parseInt(gridMode.split('x')[0]) * parseInt(gridMode.split('x')[1]) }).map((_, i) => (
                                            <div
                                                key={i}
                                                className={`bg-blue-50 border border-blue-200 flex items-center justify-center text-xs text-blue-500 font-medium ${rotation === '90cw' ? 'rotate-90' :
                                                    rotation === '90ccw' ? '-rotate-90' :
                                                        rotation === 'alternate' && i % 2 === 1 ? 'rotate-180' : ''
                                                    }`}
                                            >
                                                {i + 1}
                                            </div>
