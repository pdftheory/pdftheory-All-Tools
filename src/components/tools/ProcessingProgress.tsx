...
console.log('DEBUG: ProcessingProgress.tsx loading...');
...
export const ProcessingProgress: React.FC<ProcessingProgressProps> = ({
  ...
  const scopeClass = `progress-${uniqueId}`;
  ...
  return (
    <div
      className={`w-full ${className} ${scopeClass}`.trim()}
    ...
    >
      ...
      >
        ...
        {(status === 'uploading' || status === 'processing') && clampedProgress < 100 && (
          <div
            className="absolute left-0 top-0 h-full bg-gradient-to-r from-transparent via-white/30 to-transparent animate-shimmer w-[var(--progress)]"
          />
        )}
      </div>
