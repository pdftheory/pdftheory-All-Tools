...
export function ExtractImagesTool({ className = '' }: ExtractImagesToolProps) {
    ...
    const handleExtract = useCallback(async () => {
        ...
        try {
            ...
            if (output.success && output.metadata?.images) {
                ...
                setStatus('complete');
                logToolUsage('extract_images', files.length > 1 ? `${files.length} files` : files[0].file.name, {
                    fileCount: files.length,
                    extractedCount: (output.metadata.images as ExtractedImage[]).length,
                    minWidth,
                    minHeight,
                    minSize
                });
    ...
    }, [files, minWidth, minHeight, minSize, logToolUsage]);
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <input
                                ...
                                aria-label={tTools('extractImages.minWidth') || 'Min Width'}
                            />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-[hsl(var(--color-foreground))] mb-1">
                            ...
                            <input
                                ...
                                className="w-full px-3 py-2 border border-[hsl(var(--color-border))] rounded-[var(--radius-md)] bg-[hsl(var(--color-background))] text-[hsl(var(--color-foreground))]"
                                aria-label={tTools('extractImages.minHeight') || 'Min Height'}
                            />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-[hsl(var(--color-foreground))] mb-1">
                            ...
                            <input
                                ...
                                className="w-full px-3 py-2 border border-[hsl(var(--color-border))] rounded-[var(--radius-md)] bg-[hsl(var(--color-background))] text-[hsl(var(--color-foreground))]"
                                aria-label={tTools('extractImages.minSizeKB') || 'Min Size'}
                            />
                        </div>
                    </div>
                </Card>
