  ...
  const handleLinearize = useCallback(async () => {
    ...
    try {
      ...
      if (output.success && output.result) {
        ...
        logToolUsage('linearize_pdf', files.length === 1 ? files[0].name : `${files.length} files`, {
          fileCount: files.length,
          useObjectStreams,
          originalSize: output.metadata?.originalSize,
          linearizedSize: output.metadata?.linearizedSize
        });
      } else {
