...
export function SplitPDFTool({ className = '' }: SplitPDFToolProps) {
  ...
  }, [splitMode, rangeInput, selectedPages, totalPages, splitCount, evenOddMode, pdfBookmarks, bookmarkLabels, mergeIntoOne]);
  ...
  const handleSplit = useCallback(async () => {
    ...
    const options: SplitOptions = {
      ranges,
      outputFormat: mergeIntoOne ? 'single' : 'multiple',
    };
    ...
    try {
      ...
      if (output.success && output.result) {
        ...
        logToolUsage('split_pdf', file.name, {
  ...
  }, [file, getPageRanges, mergeIntoOne, splitMode, logToolUsage]);
      ...
      {file && totalPages > 0 && (
        <Card variant="outlined" size="lg">
          ...
          <div className="space-y-4">
            ...
            {/* Merge Option Toggle */}
            {(splitMode === 'ranges' || splitMode === 'visual' || splitMode === 'even-odd' || splitMode === 'n-times') && (
              <div className="pt-4 border-t border-[hsl(var(--color-border))]">
                <label className="flex items-center gap-3 cursor-pointer group">
                  <input
                    type="checkbox"
                    className="w-5 h-5 rounded border-[hsl(var(--color-border))] text-[hsl(var(--color-primary))] focus:ring-[hsl(var(--color-primary)/0.2)] transition-colors cursor-pointer"
