  ...
  const handleRemoveSelected = useCallback(async () => {
    ...
    try {
      ...
      if (output.success && output.result) {
        ...
        setResult(output.result as Blob);
        setStatus('complete');
        logToolUsage('edit_attachments', pdfFile.name, {
              ...
              <ul className="space-y-2" role="list">
                {attachments.map((attachment, index) => (
                  <li
                    ...
                    className={`flex items-center gap-3 p-3 rounded-[var(--radius-md)] border transition-colors ${selectedForRemoval.has(index)
