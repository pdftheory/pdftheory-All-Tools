  ...
  const handleProcess = useCallback(async () => {
    ...
    try {
      ...
      if (output.success && output.result) {
        ...
        setStatus('complete');
        logToolUsage('bookmark', file.name, {
  ...
  const renderBookmarkItem = (bookmark: BookmarkNode, depth: number = 0) => {
    ...
    return (
      <div
        ...
        className="ml-[calc(var(--depth)*16px)]"
      >
        <div
          ...
          {isEditing ? (
            <div className="flex-1 flex gap-2" onClick={(e) => e.stopPropagation()}>
              ...
              <input
                ...
                aria-label="Bookmark page number"
              />
              <Button size="sm" onClick={() => handleUpdateBookmark(editingBookmark)} aria-label="Save bookmark">✓</Button>
              <Button size="sm" variant="ghost" onClick={() => setEditingBookmark(null)} aria-label="Cancel editing">✕</Button>
            </div>
          ) : (
            <>
              ...
              {/* Actions */}
              <div className={`flex gap-1 ${isSelected ? 'opacity-100' : 'opacity-0 group-hover:opacity-100 hover:opacity-100'}`}>
                <button
