  ...
  const handleCreateNUp = useCallback(async () => {
    ...
    try {
      ...
      if (output.success && output.result) {
        ...
        setStatus('complete');
        logToolUsage('n_up_pdf', file.name, {
          fileSize: file.size,
          pagesPerSheet: pagesPerSheet === 'custom' ? `${customCols}x${customRows}` : pagesPerSheet,
          ...
          <div className="mt-6 p-4 rounded-xl bg-gradient-to-br from-[hsl(var(--color-muted))] to-[hsl(var(--color-background))] border border-[hsl(var(--color-border))]">
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-center sm:items-start">
              ...
              <div className="relative">
                <div
                  className={`relative border-2 border-[hsl(var(--color-primary)/0.3)] rounded-lg p-3 bg-white dark:bg-[hsl(var(--color-card))] shadow-sm ${orientation === 'landscape' ? 'w-[180px] h-[140px]' : 'w-[140px] h-[180px]'
                    }`}
                >
                  {/* Page label */}
