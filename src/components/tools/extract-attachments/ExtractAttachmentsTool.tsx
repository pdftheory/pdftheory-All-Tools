  ...
  const handleExtract = useCallback(async () => {
    ...
    try {
      ...
      if (output.success && output.metadata?.attachments) {
        ...
        setStatus('complete');
        logToolUsage('extract_attachments', files.length > 1 ? `${files.length} files` : files[0].file.name, {
          fileCount: files.length,
          extractedCount: (output.metadata.attachments as AttachmentInfo[]).length
        });
      } else {
