    ...
    const handleRasterize = useCallback(async () => {
        ...
        try {
            ...
            if (output.success && output.result) {
                ...
                logToolUsage('rasterize-pdf', file.name, {
                    fileSize: file.size,
                    dpi,
                    format,
                ...
                <Card variant="outlined">
                    ...
                    `}
                        ...
                        {(format === 'jpeg' || format === 'webp') && (
                            <div>
                                <label htmlFor="rasterize-quality" className="block text-sm font-medium text-[hsl(var(--color-foreground))] mb-2">
                                    {tTools('rasterizePdf.qualityLabel') || 'Quality'}: {quality}%
                                </label>
                                <input
                                    id="rasterize-quality"
                                    type="range"
                                    min="10"
                                    max="100"
                                    value={quality}
                        ...
                        {/* Background Color */}
                        <div>
                            <label htmlFor="rasterize-bg-color" className="block text-sm font-medium text-[hsl(var(--color-foreground))] mb-2">
                                {tTools('rasterizePdf.bgColorLabel') || 'Background Color'}
                            </label>
                            <div className="flex items-center gap-3">
                                <input
                                    id="rasterize-bg-color"
                                    type="color"
                                    value={backgroundColor}
                                    onChange={(e) => setBackgroundColor(e.target.value)}
                                    disabled={isProcessing}
                                    className="w-10 h-10 rounded border-0 cursor-pointer"
                                    title={tTools('rasterizePdf.bgColorLabel') || 'Background Color'}
                                />
                                <input
                                    type="text"
                                    value={backgroundColor}
                                    ...
                                    aria-label={tTools('rasterizePdf.bgColorHex') || 'Background Color Hex Code'}
                                />
                            </div>
                        </div>
                    </div>
