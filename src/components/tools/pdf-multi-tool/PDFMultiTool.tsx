...
export function PDFMultiTool({ className = '' }: PDFMultiToolProps) {
  ...
  const handleDownloadSelected = useCallback(async () => {
    ...
    try {
      ...
      URL.revokeObjectURL(url);
      logToolUsage('pdf_multi_tool', 'selected_pages.pdf', {
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      setResult(blob);
      setStatus('complete');
      logToolUsage('pdf_multi_tool', 'edited.pdf', {
        action: 'export_pdf',
                  ...
                  >
                    {page.thumbnail ? (
                      <img
                        src={page.thumbnail}
                        alt={`Page ${index + 1}`}
                        className={`w-full h-full object-contain bg-white dark:bg-slate-800 ${page.rotation === 90 ? 'rotate-90' :
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  aria-label={tTools('pdfMultiTool.numberOfPages') || 'Number of pages'}
                />
              </div>
              <div>
                <label className="block text-sm font-medium mb-2 text-[hsl(var(--color-muted-foreground))]">
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-[hsl(var(--color-border))] rounded-lg bg-[hsl(var(--color-background))]"
                  aria-label={tTools('pdfMultiTool.insertPosition') || 'Insert at position'}
                />
              </div>
