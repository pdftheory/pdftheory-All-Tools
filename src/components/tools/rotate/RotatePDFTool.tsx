  ...
  const handleRotate = useCallback(async () => {
    ...
    try {
      ...
      if (output.success && output.result) {
        ...
        setStatus('complete');
        logToolUsage('rotate_pdf', file.name, { rotated_pages: rotatedCount });
      } else {
          ...
          {isLoadingPreviews ? (
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-4 max-h-[500px] overflow-y-auto p-1">
              {pagePreviews.map((preview) => (
                ...
                >
                  <div
                    className={`relative aspect-[3/4] w-full rounded-[var(--radius-md)] border-2 overflow-hidden bg-[hsl(var(--color-muted))] ${preview.rotation !== 0
                      ? 'border-[hsl(var(--color-primary))] ring-2 ring-[hsl(var(--color-primary)/0.3)]'
                      : 'border-[hsl(var(--color-border))]'
                      }`}
                  >
                    <div
                      className="w-full h-full flex items-center justify-center transition-transform duration-300 rotate-[var(--rotation)]"
                      style={{
                        // @ts-ignore
                        '--rotation': `${preview.rotation}deg`
                      }}
                    >
                      {preview.thumbnail ? (
                        <img
