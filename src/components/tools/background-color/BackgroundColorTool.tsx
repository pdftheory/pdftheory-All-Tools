  ...
  const handleProcess = useCallback(async () => {
    ...
    try {
      ...
      if (output.success && output.result) {
        ...
        setStatus('complete');
        logToolUsage('background_color', file.name, { color });
      }
        ...
        <>
          ...
          <Card variant="outlined" size="lg">
            ...
            <div className="flex items-center gap-4">
              <input type="color" id="background-color-picker" value={color} onChange={(e) => setColor(e.target.value)} className="w-16 h-10 border rounded cursor-pointer" disabled={isProcessing} aria-label="Background color picker" />
              <input type="text" id="background-color-text" value={color} onChange={(e) => setColor(e.target.value)} className="px-3 py-2 border rounded w-32" disabled={isProcessing} aria-label="Background color hex value" />
            </div>
          </Card>
        </>
      )}
