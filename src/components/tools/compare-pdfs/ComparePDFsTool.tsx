...
export function ComparePDFsTool({ className = '' }: ComparePDFsToolProps) {
  ...
  // Update opacity without triggering re-render of canvas (to avoid inline style lint)
  useEffect(() => {
    if (canvas2Ref.current) {
      canvas2Ref.current.style.opacity = (overlayOpacity / 100).toString();
    }
  ...
  const handleCompare = useCallback(async () => {
    ...
    try {
      ...
      setCurrentPage(0);
      setStatus('complete');
      logToolUsage('compare-pdfs', `${file1.file.name} vs ${file2.file.name}`, {
        status: 'success',
        fileCount: 2,
        // Approximate total size
        fileSize: (file1.file.size + file2.file.size),
        pageCount: maxPages,
      });
    } catch (err) {
      if (!cancelledRef.current) {
      ...
      {differences.length > 0 && (
        <>
            ...
            {viewMode === 'overlay' && (
              <div className="mt-4 flex items-center gap-4">
                ...
                <input
                  ...
                  aria-label={tTools('comparePdfs.opacity') || 'Opacity'}
                />
          ...
          <Card variant="outlined">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                {currentDiff && (
                  <span className={`text-xs px-2 py-1 rounded ${currentDiff.hasDifference
          ...
          <div
            ...
            className={`border border-[hsl(var(--color-border))] rounded-[var(--radius-md)] overflow-hidden bg-[hsl(var(--color-muted)/0.2)] ${isFullscreen ? 'fixed inset-0 z-50 rounded-none border-none flex flex-col' : ''
              }`}
          >
              ...
              <div className={`grid grid-cols-1 md:grid-cols-2 gap-4 p-4 ${isFullscreen ? 'flex-1 min-h-0' : ''}`}>
                <div className={`text-center ${isFullscreen ? 'flex flex-col min-h-0' : ''}`}>
                  ...
                  <div
                    ...
                    className={`overflow-auto border border-[hsl(var(--color-border))] rounded bg-white ${isFullscreen ? 'flex-1 min-h-0' : 'max-h-[600px]'
                      }`}
                  >
            ...
            {viewMode === 'overlay' && (
              <div className={`p-4 text-center ${isFullscreen ? 'flex-1 flex flex-col min-h-0' : ''}`}>
                ...
                <div className={`relative overflow-auto border border-[hsl(var(--color-border))] rounded bg-white ${isFullscreen ? 'flex-1 min-h-0 w-full' : 'max-h-[600px] inline-block'
                  }`}>
                  <canvas ref={canvas1Ref} className="max-w-full h-auto" />
                  <canvas
                    ref={canvas2Ref}
            ...
            {viewMode === 'diff' && (
              <div className={`p-4 text-center ${isFullscreen ? 'flex-1 flex flex-col min-h-0' : ''}`}>
                ...
                <div className={`overflow-auto border border-[hsl(var(--color-border))] rounded bg-white ${isFullscreen ? 'flex-1 min-h-0 w-full' : 'max-h-[600px] inline-block'
                  }`}>
