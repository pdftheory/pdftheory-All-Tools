    ...
    try {
      ...
      if (batchMode && files.length > imagesPerPdf) {
        ...
        if (output.success && output.result) {
          ...
          setStatus('complete');
          logToolUsage('image_to_pdf', `${files.length} images`, {
            image_count: files.length,
            batch: true,
            pdf_count: output.result.pdfCount,
            total_size: output.result.zipBlob.size
          });
        } else {
        ...
        if (output.success && output.result) {
          ...
          setStatus('complete');
          logToolUsage('image_to_pdf', files[0]?.file.name || 'images', {
            image_count: files.length,
            pageSize,
            orientation,
            margin,
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <select
                ...
                aria-label={tTools('imageToPdf.pageSize') || 'Page Size'}
              >
            ...
            <div>
              ...
              <select
                ...
                disabled={isProcessing}
                className="w-full px-3 py-2 rounded-[var(--radius-md)] border border-[hsl(var(--color-border))] bg-white text-sm focus:outline-none focus:ring-2 focus:ring-[hsl(var(--color-primary))]"
                aria-label={tTools('imageToPdf.orientation') || 'Orientation'}
              >
            ...
            <div>
              ...
              <select
                ...
                disabled={isProcessing}
                className="w-full px-3 py-2 rounded-[var(--radius-md)] border border-[hsl(var(--color-border))] bg-white text-sm focus:outline-none focus:ring-2 focus:ring-[hsl(var(--color-primary))]"
                aria-label={tTools('imageToPdf.margin') || 'Margin'}
              >
              ...
              {batchMode && (
                <div className="ml-7 space-y-2">
                  ...
                  <input
                    ...
                    aria-label={tTools('imageToPdf.imagesPerPdf') || 'Images per PDF'}
                  />
                  <p className="text-xs text-[hsl(var(--color-muted-foreground))]">
          ...
          {(imageType === 'svg' || files.some(f => f.file.name.toLowerCase().endsWith('.svg') || f.file.type === 'image/svg+xml')) && (
            <div className="mt-4 pt-4 border-t border-[hsl(var(--color-border))]">
              ...
              <select
                ...
                aria-label={tTools('imageToPdf.svgQuality') || 'SVG Export Quality'}
              >
