  ...
  const handleConvert = useCallback(async () => {
    ...
    try {
      ...
      if (output.success && output.result) {
        ...
        setStatus('complete');
        const numImages = Array.isArray(output.result) ? output.result.length : 1;
        logToolUsage('pdf_to_image', file.file.name, {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            {!outputFormat && (
              <div>
                ...
                <select
                  ...
                  aria-label={tTools('pdfToImage.format') || 'Output Format'}
                >
            ...
            {['jpg', 'jpeg', 'webp'].includes(format) && (
              <div>
                ...
                <input
                  ...
                  disabled={isProcessing}
                  className="w-full"
                  aria-label={tTools('pdfToImage.quality') || 'Quality'}
                />
              </div>
            )}
            ...
            <div>
              ...
              <select
                ...
                disabled={isProcessing}
                className="w-full px-3 py-2 rounded-[var(--radius-md)] border border-[hsl(var(--color-border))] bg-white text-sm focus:outline-none focus:ring-2 focus:ring-[hsl(var(--color-primary))]"
                aria-label={tTools('pdfToImage.resolution') || 'Resolution'}
              >
              ...
              {([
                ...
                return (
                  ...
                  >
                    {/* Mini grid preview */}
                    <div
                      className={`grid gap-0.5 w-[32px] h-[24px] ${preset.value !== 'custom' ? `grid-cols-${preset.cols} grid-rows-${preset.rows}` : 'grid-cols-[repeat(var(--cols),1fr)] grid-rows-[repeat(var(--rows),1fr)]'
                        }`}
                      style={preset.value === 'custom' ? {
                        // @ts-ignore
                        '--cols': preset.cols,
                        // @ts-ignore
                        '--rows': preset.rows,
              ...
              <div className="flex gap-4 mb-4 p-3 rounded-lg bg-[hsl(var(--color-muted)/0.3)]">
                <div className="flex-1">
                  ...
                  <input
                    ...
                    disabled={isProcessing}
                    className="w-full px-3 py-2 rounded-[var(--radius-md)] border border-[hsl(var(--color-border))] bg-white text-sm focus:outline-none focus:ring-2 focus:ring-[hsl(var(--color-primary))]"
                    aria-label={tTools('pdfToImage.columns') || 'Columns'}
                  />
                </div>
                <div className="flex-1">
                  <label className="block text-xs font-medium text-[hsl(var(--color-foreground))] mb-1">
                  ...
                  <input
                    ...
                    disabled={isProcessing}
                    className="w-full px-3 py-2 rounded-[var(--radius-md)] border border-[hsl(var(--color-border))] bg-white text-sm focus:outline-none focus:ring-2 focus:ring-[hsl(var(--color-primary))]"
                    aria-label={tTools('pdfToImage.rows') || 'Rows'}
                  />
                </div>
              </div>
            )}
            ...
            {layoutPreset !== '1x1' && (
              <div className="p-4 rounded-xl bg-gradient-to-br from-[hsl(var(--color-muted))] to-[hsl(var(--color-background))] border border-[hsl(var(--color-border))]">
                ...
                <div className="flex flex-col sm:flex-row gap-4 items-center">
                  {/* Grid visualization */}
                  <div
                    className="border-2 border-[hsl(var(--color-primary)/0.3)] rounded-lg p-3 bg-white w-[140px] h-[100px]"
                  >
                    <style jsx>{`
                      .grid-preview-${uniqueId} {
                        grid-template-columns: repeat(${layoutPreset === 'custom' ? customColumns : parseInt(layoutPreset.split('x')[0])}, 1fr);
                        grid-template-rows: repeat(${layoutPreset === 'custom' ? customRows : parseInt(layoutPreset.split('x')[1])}, 1fr);
                      }
                    `}</style>
                    <div
                      className={`w-full h-full grid gap-1 grid-preview-${uniqueId}`}
                    >
