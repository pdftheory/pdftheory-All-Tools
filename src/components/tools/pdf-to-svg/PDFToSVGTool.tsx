...
export function PDFToSVGTool({ className = '' }: PDFToSVGToolProps) {
    ...
    // const { user } = useAuth(); // Removed as it was only used for history
    ...
    const handleConvert = useCallback(async () => {
        ...
        try {
            ...
            if (output.success && output.result) {
                ...
                logToolUsage('pdf_to_svg', file.file.name, {
                    page_count: (output.metadata?.svgResults as SVGResult[])?.length || 0,
                    scale,
                    pageRange
                });
    ...
    }, [file, scale, pageRange, logToolUsage]);
    ...
    return (
        <div className={`space-y-6 ${className}`.trim()}>
            ...
            {previewIndex !== null && svgResults[previewIndex] && (
                ...
                >
                    <div
                        className="relative max-w-[95%] max-h-[90vh] bg-white rounded-lg overflow-auto"
