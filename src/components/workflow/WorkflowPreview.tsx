 ...
 */
export function WorkflowPreview({ nodes, edges, inputFiles, outputFiles, isVisible, onToggle }: WorkflowPreviewProps) {
    ...
    useEffect(() => {
        ...
        generatePreview();
    }, [isVisible, inputFiles, outputFiles, nodes, edges]);
    ...
    const generatePreview = async () => {
        ...
        try {
            ...
            // If outputFiles exist, they take precedence for preview
            if (outputFiles && outputFiles.length > 0) {
                // Clear simulated previews if we have actual output
                newPreviews.length = 0;

                for (const outputFile of outputFiles) {
                    const blob = outputFile instanceof Blob ? outputFile : outputFile.blob;
                    const name = outputFile instanceof Blob ? 'Result' : outputFile.filename || 'Result';
                    ...
                    if (blob.type === 'application/pdf') {
                        ...
                        for (let i = 1; i <= maxPages; i++) {
                            ...
                            if (context) {
                                ...
                                // Note: We DON'T applyNodeEffectsToCanvas here because this IS the actual result
        ...
        >
            ...
            <div className="flex-1 overflow-hidden h-[calc(100%-44px)]">
                {inputFiles.length === 0 ? (
                  ...
                  `}
                                    title={`Select page ${preview.pageNumber} from ${preview.source}`}
                                >
                                    <img
