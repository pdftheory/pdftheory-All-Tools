...
sys.modules["fire"] = fire_mod
      ...
      // Optional: Advanced Image Processing (Pillow & OpenCV)
      // We wrap these in try-catch so core functionality works even if they fail to load
      try {
        await loadWheel(`${basePath}pillow-11.2.1-cp313-cp313-pyodide_2025_0_wasm32.whl`);
      } catch (e) {
        console.warn('Failed to load Pillow:', e);
      }

      try {
        await loadWheel(`${basePath}opencv_python-4.11.0.86-cp313-cp313-pyodide_2025_0_wasm32.whl`);
      } catch (e) {
