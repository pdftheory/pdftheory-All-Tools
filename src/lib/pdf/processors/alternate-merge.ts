  ...
  async process(
    ...
    if (files.length === 0) {
      return this.createErrorOutput(
        ...
        'No PDF files provided for alternate merging.'
      );
    }

    // If only one file, just return it (pass-through for robustness)
    if (files.length === 1) {
      this.updateProgress(100, 'Single file provided, skipping alternate merge.');
      return this.createSuccessOutput(files[0], files[0].name);
    }
