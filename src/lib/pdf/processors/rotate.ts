...
import { getPageIndices } from '../utils';
...
export interface RotateOptions {
  ...
  /** Apply rotation to specific pages only (1-based page numbers or 'all'/'odd'/'even') */
...
function getRotationsMap(options: RotateOptions, totalPages: number): Map<number, number> {
  ...
  if (options.rotations) {
    ...
    const pageIndices = getPageIndices(options.pages, totalPages);
    pageIndices.forEach(index => {
      rotationsMap.set(index + 1, options.angle!);
    });
  }
