    ...
    async process(
        ...
        if (files.length === 0) {
            return this.createErrorOutput(
                ...
                'At least 1 PDF file is required for grid combine.',
                `Received ${files.length} file(s).`
            );
        }

        // If only 1 file and pageMode is first-page-only and fillMode is leave-empty,
        // it doesn't make much sense for a grid, but we can still process it.
        // We traditionally required 2 files, but letting it work for 1 file 
        // makes the tool more robust in dynamic workflows.
        ...
        try {
            ...
            const sourcePages: { page: any; name: string; pageNum: number; doc: any }[] = [];
            for (const { doc, name } of sourceDocs) {
                ...
                if (combineOptions.pageMode === 'all-pages') {
                    ...
                    for (let i = 0; i < pages.length; i++) {
                        sourcePages.push({ page: pages[i], name, pageNum: i + 1, doc: pdfDoc });
                    }
                } else {
                    // Only first page
                    if (pages.length > 0) {
                        sourcePages.push({ page: pages[0], name, pageNum: 1, doc: pdfDoc });
                    }
                }
            }
            ...
            // NOTE: newPdf.embedPages() only supports pages from the same source document in one call.
            // We must group pages by their source doc and embed them in batches.
            this.updateProgress(45, 'Embedding pages...');

            const embeddedPageMap = new Map<any, any>();

            // Group unique pages by their parent document
            const docToPages = new Map<any, any[]>();
            for (const { page, doc } of sourcePages) {
                if (!docToPages.has(doc)) {
                    docToPages.set(doc, []);
                }
                const pagesForDoc = docToPages.get(doc)!;
                if (!pagesForDoc.includes(page)) {
                    pagesForDoc.push(page);
                }
            }

            // Embed pages for each document
            for (const [doc, pages] of docToPages.entries()) {
                const embeddedPages = await newPdf.embedPages(pages);
                for (let i = 0; i < pages.length; i++) {
                    embeddedPageMap.set(pages[i], embeddedPages[i]);
                }
            }
