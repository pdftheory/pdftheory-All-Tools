...
export class RemoveBlankPagesProcessor extends BasePDFProcessor {
  async process(input: ProcessInput, onProgress?: ProgressCallback): Promise<ProcessOutput> {
    ...
    try {
      ...
      let threshold = removeOptions.threshold || 99;

      // Normalize threshold: if it's less than 1.0, assume it's a decimal scale (0-1) and convert to percentage (0-100)
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        if (context) {
          ...
          // Fill with white background (important for transparent PDFs)
  ...
  private isPageBlank(imageData: ImageData, threshold: number): boolean {
    ...
    for (let i = 0; i < data.length; i += 4) {
      ...
      // If alpha is low, consider it transparent (which we treat as white given our background)
      // We use > 250 for more conservative detection (less likely to remove pages with light content)
