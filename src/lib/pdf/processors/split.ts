...
export class SplitPDFProcessor extends BasePDFProcessor {
  ...
  async process(
    ...
    try {
      ...
      // For 'single' output format, we'll use one document
      let singlePdf: any;
      if (splitOptions.outputFormat === 'single') {
        singlePdf = await pdfLib.PDFDocument.create();
      }
      ...
      for (let i = 0; i < splitOptions.ranges.length; i++) {
        ...
        if (splitOptions.outputFormat === 'single') {
          // Copy pages to the single document
          const copiedPages = await singlePdf.copyPages(sourcePdf, pageIndices);
          for (const page of copiedPages) {
            singlePdf.addPage(page);
          }
        } else {
          // 'multiple' format: Create a new PDF for each range
      ...
      // Handle final output for 'single' format
      if (splitOptions.outputFormat === 'single') {
        ...
        outputFilenames.push(`${baseName}_split_merged.pdf`);
      }
...
export function createSplitNTimes(totalPages: number, n: number): PageRange[] {
  // Cap n between 1 and totalPages
  const parts = Math.max(1, Math.min(n, totalPages));
  ...
  for (let i = 0; i < parts; i++) {
    ...
    // Only add if we have at least one page in this part
    if (end >= currentPage) {
